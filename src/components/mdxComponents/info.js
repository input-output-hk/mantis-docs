...
const StyledInfoBox = styled.div`
  ...
  p, h1, h2, h3, h4, span, a, button, span{margin:0}

`

const InfoBox = (props) => (
  <StyledInfoBox {...props} className={`${props.info && `info`} ${props.alert && `alert`} ${props.warn && `warn`}`}>
    {props.info && <div><Info/></div>}
    {props.warn && <div><Error/></div>}
    {props.alert && <div><ErrorCircle/></div>}
    <div>
