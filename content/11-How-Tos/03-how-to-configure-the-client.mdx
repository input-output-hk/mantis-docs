...
1. To run Mantis using the launcher, navigate to the Mantis binaries root folder (ie. mantis/target/universal/mantis-3.0) and launch with the network name:
...
### Conf file
Fast sync is enabled by default. To disable this, you can:

1. Edit the `sync.conf` by locating the following line:
`# do-fast-sync = true`

2. Then, replace 'true' with 'false':
`do-fast-sync = false`
...
> The main configuration file in Mantis is `application.conf`, which is located in `mantis/src/main/resources`. For fine tweaking, locate and edit that file.
