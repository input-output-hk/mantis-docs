...
### Can I use mantis to connect to Sagano Testnet?
  ...
  * ATM, Mantis is the only client that can connect to the Sagano Testnet.

### How fast does Mantis sync with Mainnet?

  * Approx 12 hours to full mainnet Ethereum Classic sync with fastsync.

### Who is Mantis geared towards?

  * Client: Anyone wanting to run an Ethereum Classic Node. This includes DAPP developers, community network supporters, businesses running a private or public blockchain, miners
  * Wallet: Mantis wallet is a graphical interface wallet and is geared towards ETC cryptocurrency users. Use it to send/receive transactions on chain by installing it on Windows, macOS and Linux.
