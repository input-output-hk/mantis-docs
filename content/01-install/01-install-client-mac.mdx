---
title: Install Client Apple Mac
metaTitle: Install Client Apple Mac
---

> Author's note to reviewers: Please check the installation steps. I do not have a Mac machine.

There are two Mantis applications you can use: the Mantis client and the Mantis wallet. You can download either or both.

This section describes how to install the Mantis client on your Apple Mac machine. For instructions to install the Mantis wallet, you can go [here](/install/-install-wallet-Mac).

## Prerequisites

These are the prerequisites for installing Mantis client:
* A Java Virtual Machine (JVM) with version 1.8.x.
    To check the JVM version 
    use `java -version`.
...
1. Download the Apple Mac client binary zip archive from  the [Mantis Client page](https://mantisclient.io/):
...
> Default folders for applications on MacOs are `/home/<user>/Applications` and `/home/<user>/bin`, or `/usr/local/bin`. We recommend placing the mantis folder in there.
