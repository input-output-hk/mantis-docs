...
> Please check these sentences near the end:  
>  "By default, Mantis prunes the database of state _trie_ nodes that are no longer needed."  
>  and  
"-Dmantis.network.rpc.http.enabled=_false_"

# Configuration
The Mantis client is flexible, and you have a choice of two methods for controlling it:

 - Create a custom config file
 - Configure at runtime with command-line arguments using "Dkey" flags
...
2. Create a new file called [your name].conf
...
To change configuration options at runtime on the command line, you can use Dkey arguments to define the key and value as follows:

```
./mantis-launcher [config name] -D[key name]=[value]
```

Replace `[config-name]` with your configuration file name, [key name] with the name of the key, and [value] with the new value.

You must prefix the key name with all the higher-level key names as defined in base.conf, like this:  
...
```
You can also use a Dkey parameter at runtime:
...
1. Edit the configuration file by changing the value of the following property: `do-fast-sync` to false
`do-fast-sync = false`
...
By default, Mantis prunes the database of state trie nodes that are no longer needed. You can prevent this functionality and keep all intermediate state nodes.
