...
2.  Create a new directory (its name does not matter) and initialize it. You can use npm or Yarn. Both npm and Yarn create the file `package.json`. Yarn also creates `yarn.lock`, and npm creates package-lock.json. From here on, we will demonstrate using Yarn.  
