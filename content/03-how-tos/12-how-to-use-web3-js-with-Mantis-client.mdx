...
The Ethereum web3.js interface provides many documented API calls. You can create a website or client that interacts with the Ethereum Classic blockchain.   
...
1. Make sure [node.js](https://nodejs.org/en/download/), [npm](https://www.npmjs.com/get-npm), and [yarn](https://classic.yarnpkg.com/lang/en/) are installed. There are plenty of instructional posts on the internet, so there is no need to repeat all that here.  
...
The web3.js libraries are available as an NPM package, so use NPM to install them in the directory you just created.  
```
npm install web3
```
## Code package.json
