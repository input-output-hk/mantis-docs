---
title: How to use the Web3.js interface with Mantis client
metaTitle: How to use the Web3.js interface with Mantis client
---



# Introduction  
Learn how to use the Web3 Javascript interface with the Mantis client.  
Walk through an example program that demonstrates some sample API calls.
# Running a sample application
These are the steps to create and run a web3,js application that checks the synchronization status and displays the contents of the latest block.
## Set up the file structure
1. Make sure node.js, npm, and yarn are installed. There are plenty of instructional posts on the internet, so there is no need to repeat all that here.  
2.  Create a new directory (its name does not matter) and initialize it. Yarn creates two files, `yarn.lock` and `package.json`.   
On Windows:  
 ```
...
```
## Install the web3 libraries
The web3 libraries are available as an NPM package, so use NPM to install them in the directory you just created.  
```
npm install web3
```
## Code package.json
On all systems, edit package.json so that it looks like this:
```
    {
...
Line 1 references the web3 libraries.    
...
## Test the program
Start the Mantis client. In the example, we are using the Sagano testnet.
...
```
    ...
    C:\mantis-web3-example>
```
The results above were obtained on Windows 10, 64-bit, version 20h2.
## Observe the results
...
## More information
For more information about the web3 API, see the [Read The Docs page](https://web3js.readthedocs.io/en/v1.2.4/web3.html).
